...
% force acting on the bulk eqs 32-42
...
% test eq 7
    Newt1=s.*(rhoB.*v11+rhof.*w11)+1i.*(k1.*tau11+k2.*tau21+k3.*tau31)-1;
    Newt2=s.*(rhoB.*v21+rhof.*w21)+1i.*(k1.*tau21+k2.*tau22+k3.*tau32);
    Newt3=s.*(rhoB.*v31+rhof.*w31)+1i.*(k1.*tau31+k2.*tau32+k3.*tau33);
% test eq 8
    Newf1=s.*rhof.*v11+s.*rhoE.*(w11-L*E11)-1i.*k1.*p1;
    Newf2=s.*rhof.*v21+s.*rhoE.*(w21-L*E21)-1i.*k2.*p1;
    Newf3=s.*rhof.*v31+s.*rhoE.*(w31-L*E31)-1i.*k3.*p1;
% test eq 9
