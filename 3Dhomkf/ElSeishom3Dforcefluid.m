...
% buk stress as given in eq 16
    ...
    tau22 = -2*KSStff.*GSS.*kmat22.*1i.*k1-KPPtff.*GPP.*(kmat22-1).*1i.*k1...
        - NPPtff.*GPP.*1i.*k1;
    tau31 = -KSStff.*GSS.*(2*kmat11-1).*1i.*k3-KPPtff.*GPP.*kmat11.*1i.*k3;
...
% test eq 8
    Newf1=s.*(rhof.*v11+rhoE.*(w11-L*E11))-1i.*k1.*p1-1;
