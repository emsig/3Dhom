...
% source and plots the different wave types in separate plots for 
...
v11Ps = -KPsve.*GPs11;
v11Pf = KPfve.*GPf11;
% v21 = KSSve.*(GS21-GEM31) + KPfve.*GPf21-KPsve.*GPs21;
% v31 = KSSve.*(GS31-GEM31) + KPfve.*GPf31-KPsve.*GPs31;
% electric field
...
E11Ps = -KPsee.*GPs11;
E11Pf = KPfee.*GPf11;
% E21 = KSee.*GS21-KEMee.*GEM31 + KPfee.*GPf21-KPsee.*GPs21;
% E31 = KSee.*GS31-KEMee.*GEM31 + KPfee.*GPf31-KPsee.*GPs31;
% filtration velocity
...
w11Ps = -KPswe.*GPs11;
w11Pf = KPfwe.*GPf11;
% w21 = KSwe.*GS21-KEMwe.*GEM31 + KPfwe.*GPf21-KPswe.*GPs21;
% w31 = KSwe.*GS31-KEMwe.*GEM31 + KPfwe.*GPf31-KPswe.*GPs31;
% acoustic pressure
...
H11 = 0;
H21 = KSme.*GS3-KEMme.*GEM3;
H31 = -(KSme.*GS2-KEMme.*GEM2);
% bulk stress field
KSte = -chi.*zeta.*dems;
KEMte = KSte;
...
tau21 = 2*(KSte.*(GS211-GS1/2)-KEMte.*(GEM211-GEM1/2))+KPfte.*GPf211-KPste.*GPs211;
tau31 = 2*(KSte.*(GS311-GS1/2)-KEMte.*(GEM311-GEM/2))+KPfte.*GPf311-KPste.*GPs311;
tau22 = 2.*(KSte.*(GS221-GS1)-KEMte.*(GEM221-GEM1))+KPfte.*GPf221-KPste.*GPs221 ...
      + NPfte.*GPf1-NPste.*GPs1;
tau32 = 2*KSte.*(GS321+GS321)-KEMte.*(GEM321+GS321)+KPfte.*GPf321-KPste.*GPs321;
tau33 = 2.*(KSte.*(GS331-GS1)-KEMte.*(GEM331-GEM1))+KPfte.*GPf331-KPste.*GPs331 ...
      + NPfte.*GPf1-NPste.*GPs1;
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['S-wave of G_{11}^{ve}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 SveM.eps
figure(2)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['EM field of G_{11}^{ve}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 EMveM.eps
figure(3)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['slow P-wave of G_{11}^{ve}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 PsveM.eps
figure(4)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['fast P-wave of G_{11}^{ve}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 PfveM.eps
figure(5)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['S-wave of G_{11}^{we}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 SweM.eps
figure(6)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['EM field of G_{11}^{we}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 EMweM.eps
figure(7)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['slow P-wave of G_{11}^{we}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 PsweM.eps
figure(8)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['fast P-wave of G_{11}^{we}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 PfweM.eps
figure(9)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['S-wave of G_{11}^{taue}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 SweM.eps
figure(10)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['EM field of G_{11}^{\taue}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 EMweM.eps
figure(11)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['slow P-wave of G_{11}^{\tau e}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 PsweM.eps
figure(12)
...
xlabel('x distance from source [mm]')
ylabel('y distance from source [mm]')
title(['fast P-wave of G_{11}^{\tau e}(x,y,z=5mm,t=9.8\mu','s)'])
print -deps2 PfweM.eps
