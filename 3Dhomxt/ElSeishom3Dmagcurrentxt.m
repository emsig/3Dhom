...
E11 = 0;
E12 = KSem.*GS3-KEMem.*GEM3;
E13 = -(KSem.*GS2-KEMem.*GEM2);
% E21 = -E12;
% E22 = 0;
E23 = KSem.*GS1+KEMem.*GEM1;
% E31 = -E13;
% E32 = -E23;
% E33 = 0;
% filtration velocity
...
w11 = 0;
w12 = KSwm.*GS3-KEMwm.*GEM3;
w13 = -(KSwm.*GS2-KEMwm.*GEM2);
% w21 = -w12;
% w22 = 0;
w23 = KSwm.*GS1-KEMwm.*GEM1;
% w31 = -w13;
% w32 = -w23;
% w33 = 0;
...
H12 = (KSmm.*ys.^2.*GS21 - KEMmm.*yem.^2.*GEM21)./zeta;
H13 = (KSmm.*ys.^2.*GS31 - KEMmm.*yem.^2.*GEM31)./zeta;
H22 = KSmm.*(ys.^2.*GS22./zeta-etae.*GS) - ...
      KEMmm.*(yem.^2.*GEM22./zeta-etae.*GEM) + NSSmm.*(GS - GEM);
H23 = (KSmm.*ys.^2.*GS32 - KEMmm.*yem.^2.*GEM32)./zeta;
H33 = KSmm.*(ys.^2.*GS33./zeta-etae.*GS) - ...
      KEMmm.*(yem.^2.*GEM33./zeta-etae.*GEM) + NSSmm.*(GS - GEM);
% bulk stress field
KSStm = chi.*dems;
tau111 = 0;
tau211 = KSStm.*(ys.^2.*GS31-yem.^2.*GEM31);
tau311 = -KSStm.*(ys.^2.*GS21-yem.^2.*GEM21);
tau221 = 2*KSStm.*(ys.^2.*GS32-yem.^2.*GEM32);
tau321 = -KSStm.*(ys.^2.*(GS22-GS33)-yem.^2.*(GEM22-GEM33));
tau331 = -2*KSStm.*(ys.^2.*GS32-yem.^2.*GEM32);
tau112 = -2*KSStm.*(ys.^2.*GS31-yem.^2.*GEM31);
tau212 = -KSStm.*(ys.^2.*GS32-yem.^2.*GEM32);
tau312 = -KSStm.*(ys.^2.*(GS11-GS33)-yem.^2.*(GEM11-GEM33));
tau222 = 0;
tau322 = KSStm.*(ys.^2.*GS21-yem.^2.*GEM21);
tau332 = 2*KSStm.*(ys.^2.*GS31-yem.^2.*GEM31);
tau113 = 2*KSStm.*(ys.^2.*GS21-yem.^2.*GEM21);
tau213 = -KSStm.*(ys.^2.*(GS11-GS22)-yem.^2.*(GEM11-GEM22));
tau313 = KSStm.*(ys.^2.*GS32-yem.^2.*GEM32);
tau223 = -2*KSStm.*(ys.^2.*GS21-yem.^2.*GEM21);
tau323 = -KSStm.*(ys.^2.*GS31-yem.^2.*GEM31);
tau333 = 0;
% compute and plot some time-fields
tv12=2*real(ifft(v12.*wav,mf*nf,3))/dt;
tE12=2*real(ifft(E12.*wav,mf*nf,3))/dt;
tw12=2*real(ifft(w12.*wav,mf*nf,3))/dt;
tH11=2*real(ifft(H11.*wav,mf*nf,3))/dt;
ttau113=2*real(ifft(tau113.*wav,mf*nf,3))/dt;
for it=1:nf
    imagesc(xx1,xx2,squeeze(ttau113(:,:,it)));
    colormap(1-gray)
    colorbar;
